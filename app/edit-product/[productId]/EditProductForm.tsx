...
import {
  ImageType,
  UploadedImageType,
} from "@/app/add-products/AddProductForm";
...
const EditProductForm: React.FC<EditProductFormProps> = ({
  ...
  const [images, setImages] = useState<ImageType[] | null>(product.images);
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
      ...
      try {
        for (const item of data.images) {
          if (typeof item.image !== "string") {
            ...
            // console.log("itemquanti", existedColors);
    ...
    for (let index in uploadedImages) {
      uploadedImages[index].quantity = existedColors.filter((item) => {
        return item.color === uploadedImages[index].color;
      })[0].quantity;
    }
    const productData = { ...data, images: uploadedImages };
    console.log("productData", productData);

    console.log({ id: product.id, ...productData });
    // setIsLoading(false);
    axios
      .put("/api/update-product", { id: product.id, ...productData })
      .then(() => {
            ...
            existedColors.map((item, index) => {
              return (
                <SelectColor
                  ...
                  isProductCreated={isProductEdited}
                />
              );
            })}
        </div>
